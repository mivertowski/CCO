...
export class TokenOptimizer {
   ...
   */
  private isModeratelyImportant(line: string): boolean {
  ...
  private addToCache(key: string, content: string): void {
    if (!this.strategy.cacheEnabled) return;
  ...
  private getFromCache(key: string): string | null {
    if (!this.strategy.cacheEnabled) return null;
...
export class TokenEstimator {
  private avgCharsPerToken = 4; // Approximate for English text
...
export class ContextManager {
  ...
  private priorities: Map<number, number> = new Map();
  
  constructor(maxContextSize: number = 10000) {
    this.maxContextSize = maxContextSize;
  }
  
  addContext(content: string, priority: number = 5): void {
